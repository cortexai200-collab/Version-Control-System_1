...
const RepoDetails = () => {
  ...
  const fetchIssues = async () => {
    const res = await fetch(`http://localhost:3002/issue/repo/${id}`);
    const data = await res.json();
    setIssues(data);
  };
  ...
  const handleDeleteIssue = async (issueId) => {
    await fetch(`http://localhost:3002/issue/delete/${issueId}`, {
      method: "DELETE",
    });
    fetchIssues();
  };
  ...
  const handleToggleStatus = async (issue) => {
    await fetch(`http://localhost:3002/issue/update/${issue._id}`, {
      method: "PUT",
  ...
  const handleDelete = async () => {
    await fetch(`http://localhost:3002/repo/delete/${id}`, {
      method: "DELETE",
    });
    navigate("/");
  };
  ...
  const handleToggle = async () => {
    await fetch(`http://localhost:3002/repo/toggle/${id}`, {
      method: "PATCH",
    });
    fetchRepo();
  };
